...
# ratios = [0.337, 0.167, 0.288, 0.208] #0.208
...
config = {
    ...
    "idata": idata,
    "qdata": qdata,
    "gain": 1000,          # [DAC units], scales the envelope further
    "num_periods": 1000,
    "soft_avgs": 1,
}
...
def plot_eom_spectrum(phase_codes, sample_rate, title="Expected EOM spectrum", freqs=None):
    ...
    for freq in freqs:
        ax.axvline(
            ...
            alpha=0.5,
            color = "red",
            label=f"Target: {freq/1e6:.2f} MHz"
        )
    ax.legend()
    return ax, freqs_mhz, spectrum
...
# ---------------------------------------------------------
# New plot: spectrum convolved with 1 MHz Lorentzian
# ---------------------------------------------------------
from scipy.signal import fftconvolve

# Get original spectrum
ax, fft_freqs_mhz, spectrum = plot_eom_spectrum(
    I_active,
    fs_readout,
    title="Expected serrodyne EOM spectrum (active samples only)",
    freqs=(-1) * freqs_hz
...
df = fs_readout / len(spectrum)

# 1 MHz FWHM Lorentzian
linewidth = 1e6
gamma = linewidth / 2

freqs_conv_hz = fft_freqs_mhz * 1e6

lorentzian = (
    gamma / np.pi
    / (freqs_conv_hz**2 + gamma**2)
)

# Normalize Lorentzian
lorentzian /= np.sum(lorentzian) * df

# Convolve
spectrum_convolved = fftconvolve(
    spectrum,
    lorentzian,
    mode="same"
) * df
...
ax2.plot(
    fft_freqs_mhz,
    spectrum_convolved,
    label="1 MHz Lorentzian-convolved spectrum"
)

ax2.set_xlabel("Frequency (MHz)")
ax2.set_ylabel("Magnitude")
ax2.set_title("Expected serrodyne EOM spectrum with 1 MHz linewidth")
...
for freq in freqs_hz:
    ax2.axvline(
        ...
        alpha=0.5,
        color="red",
        label=f"Target: {-1*freq/1e6:.2f} MHz"
    )
...
# Save the NEW plot
fig2.savefig(
    "/home/xilinx/jupyter_notebooks/qick/qick_demos/pictures/serrodyne_fft_shifts_1MHz_linewidth.pdf",
    bbox_inches="tight"
)
